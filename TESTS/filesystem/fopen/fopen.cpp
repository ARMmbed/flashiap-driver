...
static fsfat_kv_data_t fsfat_fopen_test_15_kv_data[] = {
...
{
    int32_t ret = 0;
    fsfat_kv_data_t *node = fsfat_fopen_test_15_kv_data;
    ...
    /* create dirs */
    node = fsfat_fopen_test_15_kv_data;
    while(node->filename != NULL) {
        ret = fsfat_filepath_make_dirs((char*) node->filename, true);
        FSFAT_TEST_UTEST_MESSAGE(fsfat_fopen_utest_msg_g, FSFAT_UTEST_MSG_BUF_SIZE, "%s:Error: failed to create dirs for filename (filename=\"%s\")(ret=%d)\n", __func__, node->filename, (int) ret);
        TEST_ASSERT_MESSAGE(ret == 0, fsfat_fopen_utest_msg_g);
...
Case cases[] = {
        ...
        Case("FSFAT_FOPEN_TEST_00: format() test.", FSFAT_FOPEN_TEST_00),
        ...
        Case("FSFAT_FOPEN_TEST_15: write/check n x 25kB data files.", FSFAT_FOPEN_TEST_15),
};
